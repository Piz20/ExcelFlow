...
namespace ExcelFlow.Services
{
    public class SendEmail
    {
        ...
        /// <param name="toRecipients">Liste des destinataires principaux.</param>
        /// <param name="ccRecipients">Liste des destinataires en copie.</param>
        /// <param name="bccRecipients">Liste des destinataires en copie cachée.</param>
        /// <param name="fromDisplayName">Nom affiché de l'expéditeur (optionnel).</param>
        /// <param name="attachmentFilePaths">Liste des fichiers joints (optionnel).</param>
        /// <param name="smtpHost">Hôte SMTP (optionnel, remplace la config).</param>
        /// <param name="smtpPort">Port SMTP (optionnel, remplace la config).</param>
        /// <param name="fromEmail">Email de l'expéditeur (optionnel, remplace la config).</param>
        /// <param name="smtpUsername">Nom d'utilisateur SMTP (optionnel).</param>
        /// <param name="smtpPassword">Mot de passe SMTP (optionnel).</param>
        /// <param name="cancellationToken">Token d'annulation.</param>
        /// <returns>True si l'email a été envoyé, false sinon.</returns>
    ...
    CancellationToken cancellationToken = default)
{
    ...
    try
    {
        ...
        {
            // Désactive la validation SSL des certificats (pas de vérification)
            client.ServerCertificateValidationCallback = (sender, certificate, chain, sslPolicyErrors) => true;

            await LogAndSend($"Connexion à l'hôte SMTP: {host}:{port}...", cancellationToken);
            await client.ConnectAsync(host, port, SecureSocketOptions.StartTls, cancellationToken);

            // Authentification : ici si nécessaire, sinon commenter ou adapter
